...
export default function Dashboard({ stats, onStart, onReset }: DashboardProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-slate-950 pt-28 pb-20 text-slate-100">
      <header className="fixed top-0 left-0 w-full p-6 flex justify-between items-center bg-slate-950/80 backdrop-blur-md z-10 border-b border-slate-800">
        <h1 className="font-bold tracking-wider text-lg uppercase text-slate-200">
          Wordbook Dashboard
        </h1>
        <button
          type="button"
          onClick={handleResetClick}
          className="px-4 py-2 rounded-full text-xs font-semibold border border-rose-900/50 text-rose-400 bg-rose-950/10 hover:bg-rose-950/30 transition-all cursor-pointer"
        >
          全ての情報をリセット
        </button>
      </header>

      <main className="flex-1 w-full max-w-4xl flex flex-col gap-8 mt-4 sm:mt-0">
        {/* グラフセクション */}
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="flex flex-col items-center p-6 rounded-2xl bg-slate-900/40 border border-slate-800/80 backdrop-blur-sm">
            <h3 className="text-sm font-medium text-slate-400 mb-4">今日めくった数</h3>
            <div className="relative w-36 h-36 mb-4">
              <Doughnut
                data={createChartData(stats.flippedToday, stats.totalWords, '#0ea5e9')}
            ...
            <p className="font-mono text-sm text-slate-300">
              <span className="text-sky-400 font-bold text-lg">{stats.flippedToday}</span>
          ...
          {/* 今日覚えた数 */}
          <div className="flex flex-col items-center p-6 rounded-2xl bg-slate-900/40 border border-slate-800/80 backdrop-blur-sm">
            <h3 className="text-sm font-medium text-slate-400 mb-4">今日覚えた数</h3>
            <div className="relative w-36 h-36 mb-4">
              <Doughnut
                data={createChartData(stats.memorizedToday, stats.totalWords, '#10b981')}
                options={chartOptions}
              />
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            ...
            <p className="font-mono text-sm text-slate-300">
              <span className="text-emerald-400 font-bold text-lg">{stats.memorizedToday}</span>
              <span className="text-slate-500 mx-1">/</span>
              {stats.totalWords} 単語
            </p>
          </div>
          ...
          {/* 今日までに覚えた数 */}
          <div className="flex flex-col items-center p-6 rounded-2xl bg-slate-900/40 border border-slate-800/80 backdrop-blur-sm">
            <h3 className="text-sm font-medium text-slate-400 mb-4">今日までに覚えた数</h3>
            <div className="relative w-36 h-36 mb-4">
              <Doughnut
                data={createChartData(stats.memorizedTotal, stats.totalWords, '#6366f1')}
                options={chartOptions}
              />
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            ...
            <p className="font-mono text-sm text-slate-300">
              <span className="text-indigo-400 font-bold text-lg">{stats.memorizedTotal}</span>
              <span className="text-slate-500 mx-1">/</span>
              {stats.totalWords} 単語
            </p>
          </div>
          ...
          <div className="flex flex-col gap-2 w-full">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                ...
                >
                  {mode === 'all' && <div className="w-2 h-2 rounded-full bg-sky-500" />}
                </div>
              </label>
              ...
              >
                ...
                <div
                  className={`w-4 h-4 rounded-full border flex items-center justify-center ${
                    mode === 'unmemorized' ? 'border-sky-500' : 'border-slate-600'
                  }`}
                >
                  {mode === 'unmemorized' && <div className="w-2 h-2 rounded-full bg-sky-500" />}
                </div>
              </label>
            </div>
          </div>
      ...
      <footer className="fixed bottom-0 w-full p-6 text-center text-slate-500 text-xs bg-slate-950/80 backdrop-blur-md border-t border-slate-900">
        <p>
